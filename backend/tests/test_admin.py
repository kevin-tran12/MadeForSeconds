...
# ── Receipt unlinking ─────────────────────────────────────────────────────────
# Receipts are tax records held under a seven-year retention policy on the
# bucket (terraform/modules/storage/buckets.tf). Detaching one from a recipe is
# an editing action; destroying it is not.

def test_admin_delete_recipe_receipt_unlinks_without_deleting_object(
    authenticated_client, mock_db, sample_recipe_doc
):
    """The URL comes off the document and the stored object is left alone."""
    kept = "https://storage.googleapis.com/r-bucket/keep.pdf"
    removed = "https://storage.googleapis.com/r-bucket/remove.pdf"
    mock_db.get.return_value = sample_recipe_doc(id="test-id", receipt_urls=[kept, removed])

    with patch("app.services.uploads.delete_gcs_blob") as raw_deleter:
        response = authenticated_client.request(
            "DELETE", "/api/admin/recipes/test-id/receipts", json={"url": removed}
        )

    assert response.status_code == 204
    mock_db.update.assert_called_once_with({"receipt_urls": [kept]})
    # The point of the change: no GCS deletion of any kind on this path.
    raw_deleter.assert_not_called()


def test_admin_delete_recipe_receipt_404s_for_url_not_on_recipe(
    authenticated_client, mock_db, sample_recipe_doc
):
    """An unrecognised URL must not silently rewrite the receipt list."""
    kept = "https://storage.googleapis.com/r-bucket/keep.pdf"
    mock_db.get.return_value = sample_recipe_doc(id="test-id", receipt_urls=[kept])

    response = authenticated_client.request(
        "DELETE",
        "/api/admin/recipes/test-id/receipts",
        json={"url": "https://storage.googleapis.com/r-bucket/other.pdf"},
    )
